<?php
	...
	if($type_of_leave !="Full Day"){
		...
		$sql1 = "INSERT into capms_admin_leave_applications (leave_ID, user_empid, user_fullname, user_dept, reason_of_leave, leave_start_date, leave_end_date, no_of_leave_days, type_of_leave, leave_message, leave_application_feedback, leave_status, remaining_pls, created_at, updated_at) VALUES (NULL, '".$emp_id."', '".$emp_name."', '".$emp_dept."', '".$reason_of_leave."', '".$start_date."', '".$end_date."', '".$total_leaves."', '".$type_of_leave."', '".$leave_message."', '', 'Pending', '".$emp_remaining_pls."', '".date('d-m-Y g:i A', strtotime('now'))."', '".date('d-m-Y g:i A', strtotime('now'))."')";
		$result1 = mysqli_query($con, $sql1);
		$last_insert_id = $con->insert_id;
		$sql2 = "INSERT into capms_admin_individual_leaves (id, leave_id, user_id, reason_of_leave, type_of_leave, leave_status, leave_date, paid_unpaid_status, created_at, updated_at) VALUES (NULL, '".$last_insert_id."', '".$emp_id."', '".$reason_of_leave."', '".$type_of_leave."', 'Pending', '".$start_date."', '', '".date('d-m-Y g:i A', strtotime('now'))."', '".date('d-m-Y g:i A', strtotime('now'))."')";
