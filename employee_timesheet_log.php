...
<head>
  ...
  <title>Timesheet Log - CERM :: Codearts Employee Relationship Management</title>
</head>
<?php
if ($_SESSION['emp_id'] == '') {
...
<?php
  ...
  if($result_user->num_rows>0) {
    while($row_user = mysqli_fetch_assoc($result_user)) {
      ...
      $user_empid = $row_user['user_empid'];
    }
  }
?>
...
<body>
        ...
        <div class="row">
          ...
          <div class="col-lg-9">
            <section class="inner-head-brd">
              <h2>Timesheet - <span class="text-primary"><?echo $user_name;?></span> (<span class="text-danger"><?echo $user_empid;?></span>)</h2>
              <ul>
  ...
  <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM8 4a.905.905 0 0 0-.9.995l.35 3.507a.552.552 0 0 0 1.1 0l.35-3.507A.905.905 0 0 0 8 4zm.002 6a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
              ...
              <button class="btn btn-primary" id="view_filled_timesheet_button">filled timesheet <span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle-fill" viewBox="0 0 16 16">
                <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM8 4a.905.905 0 0 0-.9.995l.35 3.507a.552.552 0 0 0 1.1 0l.35-3.507A.905.905 0 0 0 8 4zm.002 6a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
              </svg></span></button>
              <button class="btn btn-dark" id="accordion_button">filter timesheet <span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-exclamation-circle-fill" viewBox="0 0 16 16">
                <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0zM8 4a.905.905 0 0 0-.9.995l.35 3.507a.552.552 0 0 0 1.1 0l.35-3.507A.905.905 0 0 0 8 4zm.002 6a1 1 0 1 0 0 2 1 1 0 0 0 0-2z"/>
              </svg></span></button>
            <?php
            ...
            <section id="unfilled_timesheet" class="py-3 d-none border">
              <div class="row">
                <div class="col">
                <label for="unfilled_timesheet_dropdown" class="form-label text-danger">* unfilled timesheet of this month for current user</label>
                  <div class="row">
                  <div class="col-md-10">
                    <ul>
                    <?php
            ...
            <section id="view_filled_timesheet" class="py-3 d-none border">
              <div class="row">
                <div class="col">
                    ...
                    <div class="row">
                      <div class="col-md-10">
                        <ul>
                          ...
                          <li><a href="employee_timesheet_log.php?selected_date=<?php echo date("m/d/Y",strtotime($start_date))?>&user_id=<?php echo $_REQUEST['user_id']?>"><?php echo date("m/d/Y",strtotime($start_date))." - ".date('D - M', strtotime($start_date));?></a></li>
            ...
            <section id="timesheet_accordion" class="py-3 d-none">
              ...
              <?php
                ...
                $sql_timesheet_year = "SELECT DISTINCT SUBSTRING_INDEX(timesheet_date,'/',-1) AS timesheet_year from `capms_user_timesheet` where user_id='".$_REQUEST['user_id']."' order by timesheet_year desc;";
                $timesheet_year = '';
            ...
            <?php
              // if there is no timesheet for the day show no timesheet is filled for the day else show view-tasks section
              // $query_no_timesheet = "SELECT * FROM `capms_user_timesheet` WHERE `user_id` = '" . $_REQUEST['user_id'] . "' and `timesheet_date`= '".$current_date."'";
              $query_no_timesheet = "SELECT * FROM `capms_user_timesheet` WHERE `user_id` = '" . $_REQUEST['user_id'] . "' and `timesheet_date`= '".$current_date."'";
              $result_no_timesheet = mysqli_query($con, $query_no_timesheet);
            ?>
                ...
                <tbody>
                  ...
                  if (mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                      ...
                      $find_id_from_workload_sql = "SELECT * FROM `capms_user_workload_info` as cuwi inner join `capms_project_task_info` as cpti on cpti.task_id = cuwi.task_id inner join `capms_project_info` as cpi on cpi.project_id = cuwi.project_id WHERE cuwi.`user_id` = '" . $_REQUEST['user_id'] . "' AND `workload_id` = '" . $row['workload_id'] . "'";
  ...
  <?php foreach($timesheet_year_array as $timesheet_year) {?>
  <script>
    $(document).ready(function() {
                ...
                $.ajax({
                  ...
                  success: function(result) {
                    ...
                    for(let j=0; j<day_length; j++) {
                      ...
                      ul_str += "<div class='col-sm'><a href='employee_timesheet_log.php?user_id=<?php echo $_REQUEST['user_id']?>&timesheet_year_send="+timesheet_year_js+"&timesheet_month_send="+timesheet_month_js+"&timesheet_day_send="+day_value+"' class='btn btn-dark text-light w-100' id='"+day_value_id+"'>"+day_value+"</a></div>";
